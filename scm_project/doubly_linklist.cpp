...
node *top = NULL;
int main()//main function defined with while loop
{
    int ch,d,y;
    char ans='y';
    while(ans=='y')
...
int searchint(int x)//search function returning int value
{
int count=0;
node *searchele=top;
...
int insertdata(int x)//function to insert data
{
    if(top==NULL)
    {
        top=new node;
...
void deleteint(int x)//to delete value
{
    node *del=top;
    if(del->data == x)
    {
...
void reversel()//reverse function defined
{
node *a=top;
node *b,*c,*d;
while(a!=NULL)
